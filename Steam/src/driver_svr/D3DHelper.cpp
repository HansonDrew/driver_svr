...
#include <d3d11.h> 
#include <D3Dx11tex.h>
ID3D11DeviceContext*gpContext = NULL;
namespace D3DHelper {
	IDXGIAdapter* FindIndependent()
	{
			...
			++i)
		{
			...
			{
				std::wstring dxgi_name = desc.Description;
				if ((dxgi_name.find(L"NVIDIA") >= 0 || dxgi_name.find(L"Rendon") >= 0) && desc.DedicatedVideoMemory > 4000000)
				{
					string dx_name = WString2String(dxgi_name);
					DriverLog(dx_name.c_str());
					if (pFactory)
					{
						pFactory->Release();
					}
	...
	HRESULT SaveTextureToFile(ID3D11DeviceContext*pContext, ID3D11Texture2D* TexToSave, LPCWSTR path)
	{
		...
		hr = D3DX11SaveTextureToFile(gpContext, TexToSave, D3DX11_IFF_JPG , path);
...
#define FAILED(hr)      (((HRESULT)(hr)) < 0)
	//-----------------------------------------------------------------------------
	void CreateDevice(ID3D11Device*& device, IDXGIAdapter* pAdapter)
		//-----------------------------------------------------------------------------
	{
	   ...
	   m_pDXGIFactory->EnumAdapters(0, &pDXGIAdapter);
		...
		// Create the device and device context objectsD3D11_CREATE_DEVICE_BGRA_SUPPORT
		D3D11CreateDevice(
			nullptr,
	...
	//-----------------------------------------------------------------------------
	ID3D11Texture2D* CreateSharedMipMapTexture2D(ID3D11Device* device, DXGI_FORMAT format, int width, int height, int nSampleCount, bool keyedmutex)
		//-----------------------------------------------------------------------------
	{
		...
		if (keyedmutex)
		{
			desc.MiscFlags = D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX | D3D11_RESOURCE_MISC_GENERATE_MIPS;
		}
		else
		{
			desc.MiscFlags = D3D11_RESOURCE_MISC_SHARED | D3D11_RESOURCE_MISC_GENERATE_MIPS;
		}
		...
		HRESULT ret = device->CreateTexture2D(&desc, 0, &texture);
		if (FAILED(ret))
		{
			return NULL;
		}
    ...
    //-----------------------------------------------------------------------------
    ID3D11Texture2D* CreateSharedTexture2D(ID3D11Device* device, DXGI_FORMAT format, int width, int height, int nSampleCount, bool keyedmutex)
        //-----------------------------------------------------------------------------
    {
        ID3D11Texture2D* texture = nullptr;
	...
	//-----------------------------------------------------------------------------
	{
		...
		{
			desc.Width = width/**((height + (height / 2)))*/;
			desc.Height = height;
		}
		desc.MipLevels = 1;
        desc.MiscFlags = 0;
	...
	IDXGIKeyedMutex* AcquireMutexByTexture(ID3D11Texture2D* texture)
	{
		...
		texture->QueryInterface(__uuidof(IDXGIKeyedMutex), (void**)&pKeyedMutex);
		return pKeyedMutex;
	}
	...
	//-----------------------------------------------------------------------------
	{
		...
		if (shMap.find(hSharedTexture) != shMap.end())
		{
            ...
            if (SUCCEEDED(hr))
            {
                ...
                DriverLog("Failed to Mapp Shared Resouce!!!!!!!!!!!!!!!!!!!!!\n");
            }
		}

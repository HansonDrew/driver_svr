...
enum RemoteAudioState {
     ...
     *         以下情况下会触发回调 OnRemoteAudioStateChanged{@link #OnRemoteAudioStateChanged}：  <br>
     *       + 成功解码远端音频首帧，对应原因是：kRemoteAudioStateChangeReasonLocalUnmuted{@link #RemoteAudioStateChangeReason}  <br>
     *       + 网络由阻塞恢复正常，对应原因是：kRemoteAudioStateChangeReasonNetworkRecovery{@link #RemoteAudioStateChangeReason}  <br>
     *       + 本地用户恢复接收远端音频流，对应原因是：kRemoteAudioStateChangeReasonLocalUnmuted{@link #RemoteAudioStateChangeReason}  <br>
     *       + 远端用户恢复发送音频流，对应原因是：kRemoteAudioStateChangeReasonRemoteUnmuted{@link #RemoteAudioStateChangeReason}  <br>
     */
...
enum VideoDecoderConfig {
    kVideoDecoderConfigRaw, // 开启视频解码器，只回調解码后数据
    kVideoDecoderConfigEncode, // 关闭视频解码器，只回調解码前数据
    kVideoDecoderConfigBoth,  // 开启视频解码器，同时回調解码前、解码后数据
};

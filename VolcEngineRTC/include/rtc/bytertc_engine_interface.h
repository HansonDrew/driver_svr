...
public:
     ...
     * @brief 加入房间。用户调用此方法加入房间，同一房间内的用户间可以相互通话。  <br>
 ...
 *        创建 RTCEngine 实例后，你才可以使用 SDK 提供的其他能力。
 * @param [in] app_id  <br>
 ...
 * @notes  <br>
 *        + 如果已创建了 RTCEngine 实例且未销毁，后续再调用时，均返回这个实例。如果已创建的实例被销毁，后续再调用时，会重新创建新的实例。  <br>
 *        + 在调用 DestroyRtcEngine{@link #DestroyRtcEngine} 之后会销毁 IRtcEngine{@link #IRtcEngine} 实例，再次调用
 *          CreateRtcEngine{@link #CreateRtcEngine} 方法则会重新创建一个全新的 IRtcEngine{@link #IRtcEngine} 实例。
 */
